    ...
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <h3 className="text-gray-900 font-semibold text-sm">{c.deceased_name}</h3>
            <div className="flex gap-1">
              {institutions.slice(0, 2).map((inst) => (
                <span key={inst} className="text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded font-medium">
                  {inst}
                </span>
              ))}
            </div>
          </div>
          <p className="text-[11px] text-gray-400 mt-0.5">GW-{c.id?.slice(0, 8).toUpperCase()}</p>

          <div className="mt-3">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[11px] text-gray-500 font-medium">Recovery Progress</span>
          ...
          <div className="mt-3 space-y-1">
            {doneTasks.slice(0, 1).map((t, i) => (
              <div key={i} className="flex items-center gap-1.5 text-[11px] text-green-600">
                <span className="w-3.5 h-3.5 bg-green-100 rounded-full flex items-center justify-center text-[8px]">✓</span>
                {t.title?.slice(0, 35)}{t.title?.length > 35 ? "..." : ""}
              </div>
            ))}
            {pendingTasks.slice(0, 1).map((t, i) => (
              <div key={i} className="flex items-center gap-1.5 text-[11px] text-orange-500">
                <span className="w-3.5 h-3.5 bg-orange-100 rounded-full flex items-center justify-center text-[8px]">○</span>
                {t.title?.slice(0, 35)}{t.title?.length > 35 ? "..." : ""}
              </div>
            ))}
            {doneTasks.length === 0 && pendingTasks.length === 0 && (
              <div className="text-[11px] text-gray-400">No tasks yet — generate checklist</div>
            )}
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <nav className="h-14 bg-white border-b border-gray-200 flex items-center px-6 gap-6 sticky top-0 z-50 shadow-sm">
        <span className="text-blue-600 font-bold text-base tracking-tight flex-shrink-0">GriefWallet</span>
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          <button className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all">🔍</button>
          <button className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all">🔔</button>
          <div
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <StatCard
                ...
                icon="✅"
                badge={stats.completed_tasks > 0 ? `${Math.round((stats.completed_tasks / Math.max(stats.completed_tasks + stats.pending_tasks, 1)) * 100)}%` : null}
              ...
              <StatCard
                ...
                icon="📄"
                badge={stats.documents_generated > 0 ? `${stats.documents_generated}+` : null}
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-bold text-gray-900">Active Recovery Journeys</h2>
              ...
              {loading ? (
                ...
                <div className="bg-white border border-gray-200 border-dashed rounded-xl p-12 text-center shadow-sm">
                  ...
                  <h3 className="text-gray-900 font-semibold mb-1">No cases yet</h3>
                  <p className="text-gray-500 text-sm mb-4">Create your first recovery case to get started</p>
                  <button
                    onClick={() => navigate("/cases/new")}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {cases.map((c) => (
                    <CaseCard key={c.id} case={c} onOpen={(id) => navigate(`/cases/${id}`)} />
                  ))}
                </div>
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-bold text-gray-900">Recent Document Activity</h2>
                <button className="text-xs text-blue-600 hover:text-blue-700 font-medium">View Archive →</button>
              </div>
              <div className="bg-white border border-gray-200 rounded-xl divide-y divide-gray-100 shadow-sm">
                {cases.length === 0 ? (
                  <div className="p-6 text-center text-gray-400 text-sm">No documents generated yet</div>
                ) : (
                  cases.slice(0, 3).map((c) => (
                    <div key={c.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-all">
                      <div className="w-8 h-8 bg-red-100 rounded-lg flex items-center justify-center text-red-500 text-sm flex-shrink-0">📄</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-900 font-medium truncate">
                          Recovery_Plan_{c.deceased_name?.replace(/\s+/g, "_")}.pdf
                        </p>
                        <p className="text-xs text-gray-400">
                          Generated {c.updated_at ? new Date(c.updated_at).toLocaleDateString() : "—"} · 1.1 MB
                        </p>
                      </div>
                      <button className="text-gray-400 hover:text-gray-600 transition-all text-lg">⬇</button>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl p-5 shadow-md">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-white font-bold text-sm">AI Guardian Insights</span>
              </div>
              ...
              {cases.length > 0 && (
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-xs text-blue-200">
                    <span>⚡</span>
                    <span>Verified claim status: {stats.completed_tasks}/{stats.completed_tasks + stats.pending_tasks}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-blue-200">
                    <span>📅</span>
              ...
              <button
                onClick={() => cases.length > 0 ? navigate(`/cases/${cases[0].id}`) : navigate("/cases/new")}
                className="w-full bg-white text-blue-700 text-xs font-bold py-2 rounded-lg hover:bg-blue-50 transition-all"
              >
                {cases.length > 0 ? "Review Potential Assets" : "Start Recovery Journey"}
              </button>
            </div>

            <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
              <h3 className="text-xs text-gray-400 font-semibold uppercase tracking-wider mb-3">Quick Tools</h3>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { icon: "⬆️", label: "Upload" },
                  { icon: "✏️", label: "Draft", action: () => navigate("/cases/new") },
                  { icon: "🔗", label: "Share" },
                  { icon: "❓", label: "Help" },
                ].map((tool) => (
                  <button
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-500 font-medium">{tool.label}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      <footer className="border-t border-gray-200 mt-12 px-6 py-6 flex items-center justify-between bg-white">
        ...
        <span className="text-xs text-gray-400">© 2026 GriefWallet Financial Guardianship Services</span>
      </footer>
    </div>
  );
}